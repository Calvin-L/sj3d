...
public final class World implements MouseListener, MouseMotionListener {
	 ...
	 * @param w the width of the screen
	 * @param h the height of the screen
	 * @param settings the <code>RenderSettings</code> object to use
	 */
	public World(int w, int h, ImageObserver observer, RenderSettings settings) {
		...
		renderer = new Renderer(fullWidth, fullHeight, pixels, zbuf, modelbuf, settings);
	...
	public Model getModelAtPoint(final int x, final int y) {
		return modelbuf[(int)((float)y * settings.aaFactor * width * settings.aaFactor + (float)x * settings.aaFactor)];
	}
	...
	public Image getImage() { return finalImage; }
